...
const SideBar = ({ isActive, onClose }: SidebarProps) => {
  ...
  return (
    <>
      ...
      <aside
        className={`
          ${status ? "-translate-x-full opacity-0" : "translate-x-0 opacity-100"}
          ${isActive 
            ? "fixed z-50 w-[90%] h-auto max-h-[80vh] rounded-xl left-1/2 -translate-x-1/2 top-20 shadow-2xl" 
            : "relative w-72 h-full hidden md:flex flex-col border-r-2 border-lighthouse"}
          bg-heaven transition-all duration-300 ease-in-out overflow-hidden flex flex-col
        `}
      >
        <nav className="w-full pt-9 flex flex-col h-full justify-between pb-5">
          <div className="flex flex-col">
            <h2 className="font-medium text-xs tracking-widest text-asphalt pl-5 mb-5 uppercase">
              All Boards (3)
            </h2>
      ...
      <button
        ...
        className={`fixed left-0 bottom-20 z-40 bg-newt w-14 h-12 rounded-r-full text-white hidden md:flex items-center justify-center shadow-md transition-all duration-300 ${
      ...
      >
        ...
        <div className={`relative bg-heaven w-fit rounded-lg shadow-2xl transition-transform duration-300 ${
          isNewBoardOpen ? "scale-100 translate-y-0" : "scale-95 translate-y-10"
        }`}>
          {/* Close "X" Mark */}
          <button 
            onClick={() => setIsNewBoardOpen(false)}
